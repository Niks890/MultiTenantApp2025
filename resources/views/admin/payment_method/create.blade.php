...
@section('title', __('create_item', ['item' => strtolower(__('payment_method'))]))
@section('content')
    ...
    <section class="section">
        <div class="card">
            <div class="card-header">
                <h4 class="card-title">{{ __('create_item', ['item' => strtolower(__('payment_method'))]) }}</h4>
            </div>
            <hr class="border-dashed-form">
            <div class="card-body">
                <form action="{{ route('payment-methods.store') }}" method="POST" id="createPaymentMethodForm" novalidate>
                    @csrf
                    <div class="row">
                        <div class="col-12 form-group custom-form-group">
                            <div class="row align-items-start">
                                <label for="payment_method_name" class="col-12 col-lg-2 col-form-label custom-label text-nowrap">
                                   Thêm phương thức: <span class="text-danger">*</span>
                                </label>
                                <div class="col-12 col-lg-10">
                                    <input type="text" name="payment_method_name" id="payment_method_name"
                                        ...
                                        class="form-control" value="{{ old('payment_method_name') }}" required>
                                    <div class="invalid-feedback" id="payment_method_name_error"></div>
                                </div>
                            </div>
                        </div>
                    </div>
...
@section('js')
    ...
    <script src="{{ asset('assets/custom/js/payment-method/create.js') }}"></script>
@endsection
