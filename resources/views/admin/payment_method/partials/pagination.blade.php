<div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-3 mt-3">
    <div class="d-flex align-items-center gap-2 flex-wrap">
        ...
        <select name="paginate" id="paginate" class="form-select form-select-sm w-auto">
            ...
            <option class="all-option" value="all" {{ $selectedPaginate == 'all' ? 'selected' : '' }}>Tất cả
            </option>
        </select>
    </div>
    ...
    <div class="d-flex flex-column flex-md-row align-items-center gap-3">
        <span class="text-muted">
            Hiển thị
            từ <strong>{{ $paymentMethods->firstItem() ?? 0 }}</strong>
            đến <strong>{{ $paymentMethods->lastItem() ?? 0 }}</strong>
            trong tổng số
            <strong>{{ $paymentMethods->total() }}</strong> mẫu tin
        </span>
        <div>
