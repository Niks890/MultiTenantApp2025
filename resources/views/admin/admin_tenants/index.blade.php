...
@section('title', __('manage_item', ['item' => strtolower(__('admin_tenant'))]))

@section('content')
    <x-central.page-header title="" :breadcrumbs="[
        __('manage_item', ['item' => strtolower(__('admin_tenant'))]) => 'javascript:void(0)',
    ]" />
    ...
    <div class="card shadow-sm">
        <div class="d-flex align-items-center justify-content-start ms-4 pt-2 pb-2 mt-3 mb-4">
            <h5 class="fw-bold text-break mb-0 fs-6">
                {{ __('list_item', ['item' => strtolower(__('admin_tenant'))]) }}
            </h5>
        </div>
        ...
        <div class="d-flex justify-content-start align-items-center mt-3 mb-3 ms-4">
            <button id="addNew" type="button" class="btn btn-success" data-create-url="{{ route('admin-tenants.create') }}">
        ...
        <div class="card-body">
            <div class="card-sub bg-primary d-flex align-items-center">
                <form id="searchForm" class="d-flex align-items-center justify-content-between gap-3 w-100" method="GET"
                    ...
                    <div class="col-12">
                        <div class="row g-2 align-items-center">
                            ...
                            <div class="col-12 col-md-5">
                                {{-- select search name --}}
                                <select name="tenant_id" class="choices-select form-select">
                                    <option value="">{{ __('all_items', ['item' => strtolower(__('tenant'))]) }}
                                    </option>
                                    @foreach ($tenants as $id => $name)
                                        <option value="{{ $id }}"
                                            {{ request('tenant_id') == $id ? 'selected' : '' }}>
                                            {{ $name }}
                                        </option>
                                    @endforeach
                                </select>
...
@section('js')
    ...
    <script>
        // Translations
        const no = @json(__('no'));
        const accountName = @json(__('account_name'));
        const fullName = @json(__('full_name'));
        const email = @json(__('email'));
        const phone = @json(__('phone'));
        const address = @json(__('address'));
        const tenant = @json(__('tenant'));
    ...
    <script type="module" src="{{ asset('assets/custom/js/admin-tenants/admin-tenants.js') }}"></script>
@endsection
